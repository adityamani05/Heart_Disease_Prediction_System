...
def getAllReports(request):
    all_reports = None
    try:
        all_reports = UserReport.objects.filter(user=request.user)
    except Exception as e:
        messages.add_message(request, messages.INFO, 'No Reports found!!!.')
    print(all_reports)
    return render(request,'user_all_reports.html',{'all_reports':all_reports})
